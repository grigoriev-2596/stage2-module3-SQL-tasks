...
SELECT s.id, s.name, COUNT(*) FROM student AS s INNER JOIN payment AS p ON s.id = p.student_id WHERE EXTRACT(YYYY FROM p.payment_date) = 2018 GROUP BY s.id HAVING COUNT(p.id) > 2;
